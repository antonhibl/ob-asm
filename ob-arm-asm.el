;;; ob-arm-asm.el --- Org-babel functions for arm-64 assembly evaluation

;; Copyright (C) 2012 Hibl, Anton

;; Author: Anton Hibl <antonhibl11@gmail.com>
;; Keywords: languages
;; Homepage: http://github.com/antonhibl/ob-arm-asm
;; Version: 0.1.0
...
(add-to-list 'org-babel-tangle-lang-exts '("arm-asm" . "arm-asm"))

(defvar org-babel-default-header-args:arm-asm '())

(defvar ob-arm-asm-assemble-command "as"
  "The command to use to assemble the arm-64 assembly code.")

(defvar ob-arm-asm-link-command "ld"
...
(defmacro ob-arm-asm-as-list (val)
...
(defun org-babel-execute:arm-asm (body params)
...
Argument PARAMS parameters for org babel."
  ...
  ;; Assemble the source block using org-babel-temp-file function to create
  ;; a temporary file for the assembly code.
  (let ((assembled-file (org-babel-temp-file "arm-asm-")))
    (with-temp-file assembled-file
      (insert body)
      (shell-command (format "%s -o %s %s" ob-arm-asm-assemble-command
                             assembled-file assembled-file)))
    ;; Link the object file generated from assembling the code with
    ;; org-babel-temp-file to create a temporary executable file.
    (let ((executable-file (org-babel-temp-file "arm-asm-")))
      (shell-command (format "%s -o %s %s" ob-arm-asm-link-command
                             executable-file assembled-file))
      ;; Run the temporary executable file and return the output as results
      ;; for org-babel.
...
(defun org-babel-prep-session:arm-asm (session params)
